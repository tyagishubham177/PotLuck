...
- Keep real secrets in those local env files only; do not commit live DSNs, auth tokens, or OTLP headers into tracked files.
...
| `OTEL_EXPORTER_OTLP_PROTOCOL`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_EXPORTER_OTLP_HEADERS` | Yes if Grafana Cloud is the telemetry sink | Grafana Cloud | `apps/server/.env` | Create a Grafana Cloud stack, open OTLP ingestion, copy the endpoint URL, and copy the auth header string or token format they provide. Use `http/protobuf` when Grafana quickstart instructions call for it. |
...
5. Leave tracked example files on dummy placeholders only, even after local setup is complete.
